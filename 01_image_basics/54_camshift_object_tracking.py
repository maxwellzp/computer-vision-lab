...
def camshift_object_tracking(video_path: str):
    ...
    while True:
        ...
        rotated_rect, track_window = cv.CamShift(back_proj, track_window, term_crit)

        pts = cv.boxPoints(rotated_rect)
        pts = np.int32(pts)

        draw_frame = video_frame.copy()
        cv.polylines(draw_frame, [pts], True, (0, 255, 0), 2)

        cv.imshow("Camshift Tracking", draw_frame)
...
if __name__ == "__main__":
    ...
    camshift_object_tracking(video_path)
